...
const Message = ({ message, reaction, handleOpenModal, setModalImage, highlightedText }: { message: IMessage, reaction: Reaction[], handleOpenModal: Function, setModalImage: Function, highlightedText: string }) => {
    ...
    return (
        <>
            {
                message.message_type !== "reaction" && (<div data-id={message.id} className={`flex text-md items-end rounded-md ${isFromClient ? 'float-left justify-start' : 'float-right justify-end'} ${isFromClient && !isRead ? ' fade-out' : ''}`}>
                    <div className={
                        "max-w-xl " +
                        ...
                        {!!reaction.length &&
                            <span role="img" className={"bg-slate-100 shadow-xl absolute bottom-[-26.5px] rounded-full p-1 h-[28px] w-[28px] " + (isFromClient ? "right-0" : "left-0")}>{reaction[0].emoji}</span>
                        }
